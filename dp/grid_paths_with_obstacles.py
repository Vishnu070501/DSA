"""
Problem Statement: Unique Grid Paths With Obstacles
...
# Base Case 3: If we hit an obstacle (matrix[m][n] == -1), the path is invalid (0 paths).
# Time Complexity: O(2^(m*n)) | Space Complexity: O(m + n) for recursion stack.
def unique_grid_paths( m, n, matrix):
    if m == 0 and n == 0:
        return 1
    if m < 0 or n < 0:
        return 0
    ...
    left_move_paths = unique_grid_paths(m, n-1, matrix)
    up_move_paths = unique_grid_paths(m-1, n, matrix)
...
def unique_grid_paths_memo(m, n, matrix, dp_array=None):
    ...
    left_move_paths = unique_grid_paths_memo(m, n-1, matrix,dp_array)
    up_move_paths = unique_grid_paths_memo(m-1, n, matrix,dp_array)
...
def unique_grid_paths_tab(m, n, matrix):
    dp_array = [[None for _ in range(n+1)] for _ in range(m+1)]
    ...
    for i in range(0, m+1):
        for j in range(0, n+1):
            if dp_array[i][j] is None:
                ...
                up_val = 0 if (i>0 and matrix[i-1][j]==-1) else up_val
                left_val = 0 if (j>0 and matrix[i][j-1]==-1) else left_val
...
def unique_grid_paths_tab_opt(m, n, matrix):
...
print(f"Total Unique Paths (Matrix Output): {unique_grid_paths_memo(2, 2,
                                                                       [
                                                                           [0,0,0],
                                                                           [0,-1,0],
                                                                           [0,0,0]
